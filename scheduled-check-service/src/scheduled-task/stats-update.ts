...
export const getContainerStats = async (): Promise<ContainerStatsWithIds> => {
  ...
  try {
    ...
    // 1. Total in transit
    const { count: totalInTransit, ids: totalInTransitIds } = await getCountAndIds({
    ...
    const { count: readyForPickupInMoscow, ids: readyForPickupInMoscowIds } = await getCountAndIds({
      container_number: { $ne: null },
      store_arrive_date_update: false,
    ...
    const { count: inTransitByRailRF, ids: inTransitByRailRFIds } = await getCountAndIds({
      container_number: { $ne: null },
      store_arrive_date_update: false,
    ...
    const { count: inPortOrArrivingToPort, ids: inPortOrArrivingToPortIds } = await getCountAndIds({
      container_number: { $ne: null },
      store_arrive_date_update: false,
      etd: { $ne: null },
    ...
    const { count: inTransitBySeaRoute, ids: inTransitBySeaRouteIds } = await getCountAndIds({
      container_number: { $ne: null },
      store_arrive_date_update: false,
    ...
    const { count: inTransitByDirectRail, ids: inTransitByDirectRailIds } = await getCountAndIds({
      container_number: { $ne: null },
      store_arrive_date_update: false,
    ...
    const { count: inTransitByDirectAuto, ids: inTransitByDirectAutoIds } = await getCountAndIds({
      container_number: { $ne: null },
      store_arrive_date_update: false,
    ...
    const { count: hasCustomsDeclarationWaitingForSubmission, ids: hasCustomsDeclarationWaitingForSubmissionIds } = await getCountAndIds({
      container_number: { $ne: null },
      store_arrive_date_update: false,
      date_do_update: true,
    ...
    const { count: customsDeclarationIssued, ids: customsDeclarationIssuedIds } = await getCountAndIds({
      container_number: { $ne: null },
      store_arrive_date_update: false,
    ...
    pipeline
      ...
      .set(REDIS_KEYS.HAS_CUSTOMS_DECLARATION, hasCustomsDeclarationWaitingForSubmission)
      .set(REDIS_KEYS.CUSTOMS_DECLARATION_ISSUED, customsDeclarationIssued);

    // Save IDs to Redis sets
    if (totalInTransitIds.length) pipeline.sAdd(REDIS_ID_SETS.TOTAL_IN_TRANSIT, totalInTransitIds);
    if (readyForPickupInMoscowIds.length) pipeline.sAdd(REDIS_ID_SETS.READY_FOR_PICKUP, readyForPickupInMoscowIds);
    if (inTransitByRailRFIds.length) pipeline.sAdd(REDIS_ID_SETS.IN_TRANSIT_BY_RAIL, inTransitByRailRFIds);
    if (inPortOrArrivingToPortIds.length) pipeline.sAdd(REDIS_ID_SETS.IN_PORT, inPortOrArrivingToPortIds);
    if (inTransitBySeaRouteIds.length) pipeline.sAdd(REDIS_ID_SETS.IN_TRANSIT_BY_SEA, inTransitBySeaRouteIds);
    if (inTransitByDirectRailIds.length) pipeline.sAdd(REDIS_ID_SETS.IN_TRANSIT_BY_DIRECT_RAIL, inTransitByDirectRailIds);
    if (inTransitByDirectAutoIds.length) pipeline.sAdd(REDIS_ID_SETS.IN_TRANSIT_BY_AUTO, inTransitByDirectAutoIds);
    if (hasCustomsDeclarationWaitingForSubmissionIds.length) pipeline.sAdd(REDIS_ID_SETS.HAS_CUSTOMS_DECLARATION, hasCustomsDeclarationWaitingForSubmissionIds);
    if (customsDeclarationIssuedIds.length) pipeline.sAdd(REDIS_ID_SETS.CUSTOMS_DECLARATION_ISSUED, customsDeclarationIssuedIds);
